...
import ru.auvarova.config.BotConfig;
...
public class TelegramBot extends TelegramLongPollingBot {
    ...
    private static boolean graphFlag;
    ...
    private static final String nameFile = "График.jpg";
    ...
    private static final String ERROR_MSG_RATE = "В написании валют допущена ошибка! \nПопробуйте еще раз или начните сначала - \"/start\"";
            ...
            switch (messageText) {
                ...
                default:
                    if (Pattern.matches(REG_EX_DATE, messageText)) {
                        date = messageText.replace(" ", "");
                        selectAlgorithm(chatId);
                    } else if (graphFlag) {
                        if (new RateService().validRate(messageText.toUpperCase())) {
                            currency = messageText.toUpperCase();
                            selectPeriod(chatId);
                        } else {
                            sendMessage(chatId, ERROR_MSG_RATE);
                        }
                    } else {
    ...
    /**
     * Вывод сообщения с кнопками выбора валюты
    ...
    /**
     * Вывод сообщения с кнопками выбора периода
     * @param chatId  id текущего чата
     */
    ...
    /**
     * Вывод сообщения с кнопками выбора алгоритма
     * @param chatId  id текущего чата
     */
    ...
    /**
     * Вычисление и вывод проноза курса валют
     * @param chatId id текущего чата
     */
    private void readyForecast(long chatId) {
        String readyForecast;
        boolean currencyCountFlag = true;
        if (graphFlag) {
            String rates[] = currency.replace(" ", "").split(",");
            if (rates.length > 1)
                currencyCountFlag = false;
            ImageProcessor imageProcessor = new ImageProcessor();
            imageProcessor.printGraph(rates, countDays, algoritm, nameFile);
            sendPhoto(chatId, nameFile);
        }
        if (currencyCountFlag) {
            RateService rateService = new RateService();
            readyForecast = rateService.ExchangeRateForecast(currency, date, countDays, algoritm);
            sendMessage(chatId, readyForecast);
        }
    }
    ...
    /**
     * Метод отправки картинки
     * @param chatId id текущего чата
     * @param url местоположение изображения
     */
    private void sendPhoto(long chatId, String url) {

        InputFile photo = new InputFile(new File(url), "График");
    ...
    /**
     * Метод для перезаписи сообщения
     * @param chatId id текущего чата
     * @param messageId id сообения для перезаписи
     * @param textMessage новый текст сообщения
     */
